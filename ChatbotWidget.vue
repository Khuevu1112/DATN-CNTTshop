...
async function answerKnowledge(key, opts = {}) {
  ...
  if (entry.productHint?.keyword) {
    try {
      const results = await fetchProducts({
        keyword: entry.productHint.keyword,
      });
  ...
  if (illustrate.length > 0) {
    pushBot({
      type: "text",
      text: "Một vài sản phẩm thực tế trong shop bạn có thể tham khảo:",
    });
...
// Bổ sung specs đầy đủ nếu sản phẩm đưa vào so sánh chưa có (vd sản phẩm từ luồng
// tư vấn local, hoặc kết quả tìm kiếm rút gọn không kèm specs) — gọi chi tiết thật
// theo slug. Dùng chung cho cả 2 đường thêm vào so sánh: nút "So sánh" trên card
// (toggleCompare) và so sánh trực tiếp qua text (answerCompareQuestion).
async function ensureProductSpecs(p) {
  if (p.specs && p.specs.length > 0) return p;
  if (!p.slug) return p;
  try {
    const detail = await fetchProductBySlug(p.slug);
    if (detail) return { ...p, ...detail };
  } catch (e) {
    console.error(e);
  }
  return p;
...
async function toggleCompare(p, evt) {
  ...
  // Sản phẩm từ luồng tư vấn local (data/products.js) không có field `specs` đầy đủ
  // như kết quả API tra giá -> gọi lấy chi tiết thật (kèm specs) trước khi đưa vào
  // bảng so sánh, để so sánh luôn hiển thị đủ thông số kỹ thuật bất kể nguồn gốc thẻ.
  const full = await ensureProductSpecs(p);
  compareList.push(full);
}
...
const COMPARE_HINT_RE =
  /\b(so sanh|cai nao (tot|ngon|hon)|tot hon|ngon hon|hon (nhau|khong)|nen (mua|chon) (cai|con) nao|khac nhau (cho nao|gi))\b/;
...
function extractCompareTerms(text) {
  ...
  const parts = t
    .split(COMPARE_SPLIT_RE)
    .map((s) => s.trim())
    .filter(Boolean);
  return parts.slice(0, MAX_COMPARE); // tối đa MAX_COMPARE vế (hỗ trợ so sánh 3 sản phẩm/1 câu)
}

// Thử tìm sản phẩm theo từ khóa đầy đủ trước; nếu rỗng (do backend search khớp
// nguyên cụm, không phải từng từ), rút gọn dần cụm từ đến khi ra kết quả hoặc hết từ.
// VD "ram corsair 16gb" không khớp gì -> thử "ram corsair" -> vẫn ra đúng RAM Corsair.
async function fetchProductsWithFallback(term) {
  const words = term.split(" ").filter(Boolean);
  for (let len = words.length; len >= 1; len--) {
    const attempt = words.slice(0, len).join(" ");
    try {
      const results = await fetchProducts({ keyword: attempt });
...
async function answerCompareQuestion(text) {
  ...
  if (validTerms.length < 2) {
    pushBot({
      type: "text",
      text: 'Bạn cho mình biết cụ thể các sản phẩm muốn so sánh nhé, ví dụ: "RAM 16GB DDR4 với RAM 32GB DDR5" (so sánh được tối đa 3 sản phẩm trong 1 câu).',
    });
    pushBot({ type: "quick-replies", options: MAIN_MENU });
    step.value = "menu";
    return;
  ...
  const namesPreview = validTerms.map((t) => `"${t}"`).join(", ");
  const loadingMsg = pushBot(
    { type: "text", text: `Đang tìm ${namesPreview} để so sánh...` },
    { skipPersist: true }
  );
  ...
  try {
    results = await Promise.all(
      validTerms.map((term) => fetchProductsWithFallback(term))
    );
  } catch (e) {
    console.error(e);
  }
  ...
  const idx = messages.findIndex((m) => m.id === loadingMsg.id);
  const found = validTerms.map((term, i) => ({
    term,
    product: (results[i] || [])[0],
  }));
  const missing = found.filter((f) => !f.product);
  const okItems = found.filter((f) => f.product);

  let finalText = "";
  if (okItems.length < 2) {
    const missingNames = missing.map((f) => `"${f.term}"`).join(", ");
    finalText = `Mình không tìm thấy đủ dữ liệu cho ${missingNames}. Bạn thử gõ tên chính xác hơn nhé.`;
  } else {
    clearCompare();
    const withSpecs = await Promise.all(
      okItems.map((f) => ensureProductSpecs(f.product))
    );
    withSpecs.forEach((p) => compareList.push(p));
    const productNames = okItems.map((f) => `"${f.product.name}"`).join(", ");
    finalText = `Mình đã đưa ${productNames} vào bảng so sánh bên cạnh, bạn xem chi tiết nhé.`;
    if (missing.length > 0) {
      finalText += ` (Không tìm thấy: ${missing
        .map((f) => `"${f.term}"`)
        .join(", ")})`;
    }
  }
...
// Gộp toàn bộ specs của các sản phẩm đang so sánh thành các hàng chung — mỗi hàng là
// 1 spec key (vd "CPU", "RAM"...) xuất hiện ở ít nhất 1 sản phẩm, giá trị "—" nếu sản
// phẩm đó không có spec key này. Giá/giá gốc/thương hiệu luôn đứng đầu, sau đó tới specs.
const compareRows = computed(() => {
  ...
  const rows = [
    ...
    },
    {
      ...
      values: compareList.map(
        (p) => p.brandName || p.brand || p.categoryName || "—"
      ),
    },
  ];

  // Thu thập toàn bộ spec key xuất hiện ở bất kỳ sản phẩm nào, giữ thứ tự xuất hiện.
  const specKeys = [];
  compareList.forEach((p) => {
    (p.specs || []).forEach((s) => {
      if (s.key && !specKeys.includes(s.key)) specKeys.push(s.key);
    });
  });
  ...
  specKeys.forEach((key) => {
    rows.push({
      label: key,
      values: compareList.map((p) => {
        const found = (p.specs || []).find((s) => s.key === key);
        return found?.value || "—";
      }),
    });
  });
...
// ---------- gợi ý nên mua sản phẩm nào dựa trên bảng so sánh ----------
// So sánh giá (thấp hơn = lợi) và các spec dạng số parse được (RAM GB, Hz, dung lượng...).
// Chỉ đưa gợi ý khi có sự khác biệt RÕ RỆT để tránh khuyên bừa khi các sản phẩm ngang nhau.
function parseNumericSpec(value) {
...
const compareRecommendation = computed(() => {
  ...
  // Điểm mỗi sản phẩm: +1 nếu giá thấp nhất, +1 cho mỗi spec số cao nhất (giả định
  // "cao hơn thì tốt hơn" — hợp lý với đa số spec linh kiện: RAM, Hz, VRAM, số nhân...).
  const scores = compareList.map(() => 0);

  const prices = compareList.map((p) => p.price || Infinity);
  const minPrice = Math.min(...prices);
  prices.forEach((price, i) => {
    if (price === minPrice) scores[i] += 1;
  });
  ...
  specKeys.forEach((key) => {
    const nums = compareList.map((p) => {
      const found = (p.specs || []).find((s) => s.key === key);
      return parseNumericSpec(found?.value);
    });
    const validNums = nums.filter((n) => n !== null);
    if (validNums.length < 2) return; // không đủ dữ liệu số để so sánh cho spec này

    const maxNum = Math.max(...validNums);
    nums.forEach((n, i) => {
      if (n !== null && n === maxNum) scores[i] += 1;
    });
  });
  ...
  // Không đưa gợi ý nếu tất cả sản phẩm hoà điểm (không đủ khác biệt để khuyên).
  if (winners.length === compareList.length || maxScore === 0) return null;

  if (winners.length === 1) {
    const winner = compareList[winners[0]];
    return `→ Gợi ý: "${winner.name}" có vẻ nhỉnh hơn về giá/thông số trong nhóm này, đáng cân nhắc nhất.`;
  }

  const names = winners.map((i) => `"${compareList[i].name}"`).join(" và ");
  return `→ Gợi ý: ${names} đang ngang ngửa nhau, bạn cân nhắc thêm theo nhu cầu cụ thể.`;
});
...
async function sendFreeText() {
  ...
  if (isClarifyQuestion(text) && lastKnowledgeKey.value) {
    ...
    pushBot({
      ...
      text:
        simple ||
        "Mình xin lỗi, bạn hỏi cụ thể hơn giúp mình chỗ nào chưa rõ nhé.",
    });
    pushBot({ type: "quick-replies", options: MAIN_MENU });
    step.value = "menu";
    return;
    ...
    <div v-if="open" class="cbw-panels">
      ...
      <div v-if="compareList.length >= 1" class="cbw-compare-panel">
        <div class="cbw-compare-head">
          <span
            >So sánh {{ compareList.length }}/{{ MAX_COMPARE }} sản phẩm</span
          >
          <button class="cbw-compare-clear" @click="clearCompare">
            Xoá hết
          </button>
        </div>
        ...
        <div class="cbw-compare-list">
          <div
            v-for="(p, ci) in compareList"
            :key="compareKey(p)"
            class="cbw-compare-item"
          >
            <div class="cbw-compare-item-head">
              <span class="cbw-compare-pname" @click="openProduct(p)">{{
                p.name
              }}</span>
              <button
                class="cbw-compare-remove"
        ...
        <div v-if="compareRecommendation" class="cbw-compare-recommendation">
        ...
        <div class="cbw-header">
          ...
          <div class="cbw-sub">
            ...
            <span
              v-if="state.user"
              class="cbw-synced"
              title="Lịch sử chat đang được lưu"
              >· đã lưu lịch sử</span
            >
          </div>
        </div>
