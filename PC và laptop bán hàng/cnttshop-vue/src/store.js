...
import { accents, products, productById, defaultSel, priceWith, cfgKey } from './data/products.js'
...
export const actions = {
  ...
  goCart: () => go({ route: 'cart', placed: false }, 480, 'Đang mở giỏ hàng...'),
  goCat: (cat) => go({ route: 'category', cat, brandFilter: [], q: '' }, 480, 'Đang tải sản phẩm...'),
  goCatAll: () => actions.goCat('all'),
  goCatPC: () => actions.goCat('pc'),
  goCatLap: () => actions.goCat('laptop'),
  goDetail: (id) => {
    const p = productById(id)
    go({ route: 'detail', selId: id, cfgSel: defaultSel(p) }, 480, 'Đang tải chi tiết...')
  },

  setTheme: (t) => { state.theme = t },
  toggleMode: () => { state.mode = state.mode === 'light' ? 'dark' : 'light' },
  setCfg: (groupKey, idx) => { state.cfgSel = { ...state.cfgSel, [groupKey]: idx } },

  showToast: (msg) => {
    state.toast = msg
    clearTimeout(_toastTimer)
    _toastTimer = setTimeout(() => { state.toast = '' }, 1900)
  },
  ...
  inc: (key) => { const l = state.cart.find(x => x.key === key); if (l) l.qty += 1 },
  dec: (key) => { const l = state.cart.find(x => x.key === key); if (l) l.qty = Math.max(1, l.qty - 1) },
  removeLine: (key) => { state.cart = state.cart.filter(x => x.key !== key) },

  placeOrder: () => go({ placed: true }, 950, 'Đang xử lý đơn hàng...'),
  resetShop: () => go({ cart: [], placed: false, route: 'home' }, 480, 'Đang về trang chủ...'),

  setQ: (v) => { state.q = v },
  onSearchEnter: () => actions.goCat('all'),
  setSort: (v) => { state.sort = v },
  setPriceMax: (v) => { state.priceMax = Number(v) },
  toggleBrand: (b) => {
...
export const cartCount = computed(() => state.cart.reduce((a, c) => a + c.qty, 0))

export const themeStyle = computed(() => ({
  pageBg: state.mode === 'light'
    ? '#ffffff'
