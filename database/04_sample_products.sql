-- ============================================================
--  DỮ LIỆU MẪU (tùy chọn) — 7 sản phẩm + biến thể + ảnh
--  Chạy SAU 01_DATNCNTTSHOP.sql để trang chủ Vue có hàng hiển thị.
--  Ảnh dùng placehold.co (chỉ để demo).
--  (Đã bỏ "iPhone 15 Pro" vì danh mục "Điện thoại" đã bị xoá khỏi 01.)
-- ============================================================
USE ShopDB;
GO

IF NOT EXISTS (SELECT 1 FROM PRODUCT WHERE slug = 'laptop-asus-tuf-f15')
...
IF NOT EXISTS (SELECT 1 FROM PRODUCT_VARIANT WHERE sku = 'SKU-ASUS-TUF')
...
IF NOT EXISTS (SELECT 1 FROM PRODUCT_IMAGE pi JOIN PRODUCT p ON pi.product_id=p.id WHERE p.slug='laptop-asus-tuf-f15')
...
PRINT N'✅ Đã thêm 7 sản phẩm mẫu.';
GO
