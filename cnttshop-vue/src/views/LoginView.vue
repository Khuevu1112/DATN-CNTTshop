    ...
    <div style="flex:1; display:grid; grid-template-columns:1.15fr 1fr; align-items:stretch;">
      ...
      <aside :style="{ boxShadow: '0 0 110px -10px color-mix(in srgb, ' + accent + ' 40%, transparent)' }" style="position:relative; overflow:hidden; margin:28px; border-radius:28px; background:linear-gradient(150deg,#1e2024 0%, #101114 75%); display:flex; flex-direction:column; justify-content:center; padding:64px 64px; min-height:520px;">
        ...
        <div style="position:relative; max-width:460px;">
          ...
          <h1 style="font-family:'Be Vietnam Pro',sans-serif; font-weight:800; font-size:44px; line-height:1.1; margin:0 0 18px; letter-spacing:-1px; color:#fff;">Chào mừng<br />trở lại.</h1>
      ...
      <section style="display:flex; align-items:center; justify-content:center; padding:48px 56px;">
        <div :style="{ background: cardBg, borderColor: cardBorder }" style="width:100%; max-width:460px; border:1px solid; border-radius:22px; padding:44px 40px;">
          ...
          <form v-if="!fp.step && tab.v === 'login'" @submit.prevent="submitLogin">
            <h2 :style="{ color: ink }" style="font-family:'Be Vietnam Pro',sans-serif; font-weight:700; font-size:26px; margin:0 0 8px;">Đăng nhập tài khoản</h2>
            ...
            <input v-model="login.id" type="text" placeholder="vd: 0912 345 678" :style="{ background: inputBg, borderColor: inputBorder, color: ink }" class="lp-input" style="width:100%; height:54px; padding:0 16px; border-radius:12px; border:1px solid; font-size:15px; font-family:'Be Vietnam Pro',sans-serif; margin-bottom:20px;" />

            <label :style="{ color: inkSoft }" style="display:block; font-size:13px; font-weight:600; margin-bottom:8px;">Mật khẩu</label>
            <div style="position:relative; margin-bottom:16px;">
              <input v-model="login.pw" :type="login.showPw ? 'text' : 'password'" placeholder="Nhập mật khẩu" :style="{ background: inputBg, borderColor: inputBorder, color: ink }" class="lp-input" style="width:100%; height:54px; padding:0 48px 0 16px; border-radius:12px; border:1px solid; font-size:15px; font-family:'Be Vietnam Pro',sans-serif;" />
          ...
          <form v-else-if="!fp.step" @submit.prevent="submitRegister">
            <h2 :style="{ color: ink }" style="font-family:'Be Vietnam Pro',sans-serif; font-weight:700; font-size:26px; margin:0 0 8px;">Tạo tài khoản mới</h2>
            ...
            <input v-model="reg.name" type="text" placeholder="Nguyễn Văn A" :style="{ background: inputBg, borderColor: inputBorder, color: ink }" class="lp-input" style="width:100%; height:54px; padding:0 16px; border-radius:12px; border:1px solid; font-size:15px; font-family:'Be Vietnam Pro',sans-serif; margin-bottom:18px;" />

            <label :style="{ color: inkSoft }" style="display:block; font-size:13px; font-weight:600; margin-bottom:8px;">Email</label>
            <input v-model="reg.id" type="email" placeholder="vd: ban@email.com" :style="{ background: inputBg, borderColor: inputBorder, color: ink }" class="lp-input" style="width:100%; height:54px; padding:0 16px; border-radius:12px; border:1px solid; font-size:15px; font-family:'Be Vietnam Pro',sans-serif; margin-bottom:18px;" />

            <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:18px;">
              <div>
                <label :style="{ color: inkSoft }" style="display:block; font-size:13px; font-weight:600; margin-bottom:8px;">Mật khẩu</label>
                <input v-model="reg.pw" :type="reg.showPw ? 'text' : 'password'" placeholder="Tối thiểu 8 ký tự" :style="{ background: inputBg, borderColor: inputBorder, color: ink }" class="lp-input" style="width:100%; height:54px; padding:0 16px; border-radius:12px; border:1px solid; font-size:15px; font-family:'Be Vietnam Pro',sans-serif;" />
              </div>
              <div>
                <label :style="{ color: inkSoft }" style="display:block; font-size:13px; font-weight:600; margin-bottom:8px;">Xác nhận</label>
                <input v-model="reg.pw2" :type="reg.showPw ? 'text' : 'password'" placeholder="Nhập lại mật khẩu" :style="{ background: inputBg, borderColor: inputBorder, color: ink }" class="lp-input" style="width:100%; height:54px; padding:0 16px; border-radius:12px; border:1px solid; font-size:15px; font-family:'Be Vietnam Pro',sans-serif;" />
              </div>
            </div>
          ...
          <div v-else>
            ...
            <template v-if="fp.step === 'email'">
              <h2 :style="{ color: ink }" style="font-family:'Be Vietnam Pro',sans-serif; font-weight:700; font-size:24px; margin:0 0 8px;">Quên mật khẩu</h2>
              ...
              <form @submit.prevent="fpSendOtp">
                ...
                <input v-model="fp.email" type="email" placeholder="email@gmail.com" :style="{ background: inputBg, borderColor: inputBorder, color: ink }" class="lp-input" style="width:100%; height:54px; padding:0 16px; border-radius:12px; border:1px solid; font-size:15px; font-family:'Be Vietnam Pro',sans-serif; margin-bottom:22px;" />
                <button type="submit" :disabled="fp.loading" :style="{ background: accent }" class="lp-submit" style="width:100%; height:56px; border:none; border-radius:13px; color:var(--acc-ink); font-family:'Be Vietnam Pro',sans-serif; font-weight:700; font-size:16px; cursor:pointer;">{{ fp.loading ? 'Đang gửi...' : 'Gửi mã xác nhận' }}</button>
              </form>
            </template>

            <template v-else-if="fp.step === 'otp'">
              <h2 :style="{ color: ink }" style="font-family:'Be Vietnam Pro',sans-serif; font-weight:700; font-size:24px; margin:0 0 8px;">Nhập mã xác nhận</h2>
              ...
              <form @submit.prevent="fpVerifyOtp">
                ...
                <button type="submit" :disabled="fp.loading" :style="{ background: accent }" class="lp-submit" style="width:100%; height:56px; border:none; border-radius:13px; color:var(--acc-ink); font-family:'Be Vietnam Pro',sans-serif; font-weight:700; font-size:16px; cursor:pointer;">{{ fp.loading ? 'Đang xác nhận...' : 'Xác nhận' }}</button>
              </form>
            </template>

            <template v-else-if="fp.step === 'reset'">
              <h2 :style="{ color: ink }" style="font-family:'Be Vietnam Pro',sans-serif; font-weight:700; font-size:24px; margin:0 0 8px;">Đặt mật khẩu mới</h2>
              ...
              <form @submit.prevent="fpResetPassword">
                ...
                <input v-model="fp.pw" type="password" placeholder="••••••••" :style="{ background: inputBg, borderColor: inputBorder, color: ink }" class="lp-input" style="width:100%; height:54px; padding:0 16px; border-radius:12px; border:1px solid; font-size:15px; font-family:'Be Vietnam Pro',sans-serif; margin-bottom:14px;" />
                ...
                <input v-model="fp.pw2" type="password" placeholder="••••••••" :style="{ background: inputBg, borderColor: inputBorder, color: ink }" class="lp-input" style="width:100%; height:54px; padding:0 16px; border-radius:12px; border:1px solid; font-size:15px; font-family:'Be Vietnam Pro',sans-serif; margin-bottom:22px;" />
                <button type="submit" :disabled="fp.loading" :style="{ background: accent }" class="lp-submit" style="width:100%; height:56px; border:none; border-radius:13px; color:var(--acc-ink); font-family:'Be Vietnam Pro',sans-serif; font-weight:700; font-size:16px; cursor:pointer;">{{ fp.loading ? 'Đang lưu...' : 'Đổi mật khẩu' }}</button>
              </form>
            </template>
